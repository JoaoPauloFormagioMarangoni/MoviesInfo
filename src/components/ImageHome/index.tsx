...
export function ImageHome() {
  ...
  const [firstMovie, setFirstMovie] = useState(data)
  ...
  return (
    <Container>
      <img src={IMG_API + firstMovie[0].backdrop_path} alt="" />
      <div>
        <h2>{firstMovie[0].title}</h2>
        <div>
          <span>{firstMovie[0].vote_average}</span>
        ...
        <button onClick={() => handleSelectMovie(firstMovie[0].id)}>
